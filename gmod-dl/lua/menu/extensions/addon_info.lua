...
function PANEL:Update( addonname )

	local info = GetAddonInfo( addonname )
		
	self.Name:SetText( info.Name )
	
	if ( string.Trim( info.Author ) != "" ) then
		self.Author:SetText( "By "..info.Author )
	else
		self.Author:SetText( "none" )
	end
	
	self.Info:SetText( info.Info )
	self.LastUpdate:SetText( info.LastUpdate )
	self.Version:SetText( string.format( "%.2f", tonumber( info.Version ) or 1 ) )
	
	if ( info.Email == "" ) then info.Email = "none" end
	self.Email:SetText( info.Email )
	
	if ( info.URL == "" ) then info.URL = "http://www.garrysmod.com" end
	self.URL:SetText( info.URL )
	self.URL:SetURL( info.URL )
