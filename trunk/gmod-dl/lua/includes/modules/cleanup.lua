...
if ( SERVER ) then
	...
	concommand.Add( "gmod_cleanup",		CC_Cleanup )
	concommand.Add( "gmod_admin_cleanup",	CC_AdminCleanup )
	
else

	function UpdateUI()
	
		if ( !GetControlPanel) then
			debug.Trace()
		end
	
		local Panel = GetControlPanel( "User_Cleanup" )
		...
		local Panel = GetControlPanel( "Admin_Cleanup" )
		if (!Panel) then return end
